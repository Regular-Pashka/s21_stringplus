...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  while (*format) {
    if (*format == '%') {
      ...
      format = specs_define(
          &specs, format);  // на этом этапе считываем все спецификаторы перег
                            // спецификатором-буквой
      ...
      for (int i = 0; buffer_str[i]; i++, str++) *str = buffer_str[i];
...
const char *specs_define(Spec *specs,
                         const char *format) {  // определяем спецификаторы пред
...
const char *width_precicion_define(const char *format, Spec *specs,
                                   va_list arguments) {
  // width definition
  if (*format == '*') {
    format++;
...
void processing(Spec *specs, char *buffer_str, va_list arguments) {
  switch (specs->specifier) {
    case 'd':
    case 'i':
      integer_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'u':
      unsigned_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'o':
      octal_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'x':
    case 'X':
      hex_strategy(&(*specs), buffer_str, arguments);
      break;
    case '%':
      *buffer_str = '%';
      break;
    case 'c':
      char_strategy(&(*specs), buffer_str, arguments);
      break;
    case 's':
      string_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'p':
      pointer_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'f':
      float_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'e':
    case 'E':
      mantiss_strategy(&(*specs), buffer_str, arguments);
      break;
    case 'g':
    case 'G':
      gG_strategy(&(*specs), buffer_str, arguments);
      break;
  }
}
...
void precision_processing(Spec *specs, char *buffer_str) {
  ...
  if ((specs->specifier == 'd') || (specs->specifier == 'i') ||
      (specs->specifier == 'o') || (specs->specifier == 'u') ||
      (specs->specifier == 'x') || (specs->specifier == 'X')) {
    if (s21_strlen(buffer_str) >= 1) {
      if (specs->precision == 0 && specs->is_precision_set == 1 &&
          *buffer_str == '0') {
        *buffer_str = '\0';
      } else if (specs->precision != 0 && specs->is_precision_set == 1 &&
                 *buffer_str == '0') {
        for (int i = 0; i < specs->precision - 1; i++) tmp_buff[i] = '0';
...
void flags_processing(Spec *specs, char *buffer_str) {
  ...
  if (specs->width > (int)s21_strlen(buffer_str) && !(specs->minus)) {  // !
    ...
    for (int i = (int)s21_strlen(tmp_tmp_buff); i < specs->width - initial_len;
         i++) {
      if (!specs->zero)
        tmp_tmp_buff[i] = ' ';
      else
  ...
  } else if (specs->width > (int)s21_strlen(buffer_str) && specs->minus &&
             !specs->plus) {  //!
  ...
  if (specs->hash == 1 && (specs->specifier == 'x' || specs->specifier == 'X' ||
                           specs->specifier == 'o')) {
    tmp_buff[0] = '0';
...
void unsigned_to_array(long val, char *buff, int base) {
  ...
  for (int j = 0; buf[idx + 1]; idx++, j++) buff[j] = buf[idx + 1];
}
  ...
  if (specs->length == 'l') {
    ...
    if (!specs->minus && specs->width) {
      for (int i = 0; i < specs->width; i++) {
        buffer_str[i] = ' ';
        if (i == specs->width - 1) buffer_str[i] = c;
      }
    } else if (specs->width) {
      buffer_str[0] = c;
...
void string_strategy(Spec *specs, char *buffer_str, va_list arguments) {
  if (specs->length == 'l') {
    ...
    if (specs->is_precision_set) tmp[specs->precision] = '\0';
...
void double_to_string(long double val, char *buffer_str, Spec f) {
  ...
  if ((f.is_precision_set && f.precision != 0) || (int)r ||
      ...
      (f.is_precision_set && f.hash && f.precision == 0))
    buff[idx--] = '.';
  if (!left) {
    buff[idx] = '0';
    idx--;
...
void gG_strategy(Spec *specs, char *buffer_str, va_list arguments) {
  ...
  for (; (int)val / 10 != 0; pow++, val /= 10)
    ;
  if (specs->precision == 0 && specs->is_precision_set)
    specs->precision = 1;
  else if (specs->precision == 0 && !specs->is_precision_set)
    specs->precision = 6;
  if (pow < -4 || pow >= specs->precision - 1) {
    ...
    if (!specs->hash) remove_trailing_zeroes(buffer_str);
    mantiss(buffer_str, pow, sign, specs);
  } else {
    specs->specifier = 'f';
    flag_g_f = 1;
